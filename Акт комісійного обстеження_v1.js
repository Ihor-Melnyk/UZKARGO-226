...
function onTaskExecutedEnterResults(routeStage) {
  ...
  if (routeStage.executionResult == "executed") {
    ...
    if (!EdocsApi.getAttributeValue("Number").value) throw `Внесіть значення в поле "Номер протоколу, яким прийняте рішення"`;
    if (!EdocsApi.getAttributeValue("DateProtocol").value) throw `Внесіть значення в поле "Дата протоколу, яким прийняте рішення"`;
    setRegistration();
  }
}
...
function AddDatAndNumberTask() {
  ...
  if (stateTask == "assigned" || stateTask == "inProgress" || stateTask == "delegated") {
    ...
    setPropertyDisabled("Date", false);
    setPropertyHidden("NumberProtocol", false);
    setPropertyHidden("Date", false);
  } else if (stateTask == "completed") {
    setPropertyDisabled("NumberProtocol");
    setPropertyDisabled("Date");
    setPropertyHidden("NumberProtocol", false);
    setPropertyHidden("Date", false);
  } else {
    setPropertyDisabled("NumberProtocol", false);
    ...
    setPropertyHidden("Date");
  }
}

function onTaskExecuteAddDatAndNumber(routeStage) {
  if (routeStage.executionResult == "executed") {
    if (!EdocsApi.getAttributeValue("Date").value) throw `Внесіть значення в поле "Дата протоколу АТ "УЗ""`;
    if (!EdocsApi.getAttributeValue("NumberProtocol").value) throw `Внесіть значення в поле "Номер протоколу АТ "УЗ""`;
    setRegistration();
  }
}

function FixedVersionTask() {
  var stateTask = EdocsApi.getCaseTaskDataByCode("FixedVersion")?.state;
  if (stateTask == "assigned" || stateTask == "inProgress" || stateTask == "delegated") {
    EdocsApi.message(
...
function ProcessActTask() {
  ...
  } else if (stateTask == "completed" || stateTask == "rejected") {
...
function onChangeHeadCommission() {
  ...
  if (HeadCommission) {
    var employee = EdocsApi.getEmployeeDataByEmployeeID(toString(HeadCommission));
...
function setRegistration() {
  switch (EdocsApi.getAttributeValue("DocKind")?.value) {
    case `Акт з введення в експлуатацію об’єктів будівництва рівня АТ "УЗ"`:
      EdocsApi.setAttributeValue({ code: "RegDate", value: EdocsApi.getAttributeValue("Date")?.value, text: null });
      EdocsApi.setAttributeValue({ code: "RegNumber", value: EdocsApi.getAttributeValue("NumberProtocol")?.value, text: null });
      break;

    default:
      EdocsApi.setAttributeValue({ code: "RegDate", value: EdocsApi.getAttributeValue("DateProtocol")?.value, text: null });
      EdocsApi.setAttributeValue({ code: "RegNumber", value: EdocsApi.getAttributeValue("Number")?.value, text: null });
      break;
  }
}
